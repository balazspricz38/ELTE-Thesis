...
\subsubsection{A \texttt{dashboard} modul tesztjei}
...
\subsubsection{A \texttt{processor} modul tesztjei}
...
\subsubsection{A \texttt{dictionary} modul tesztjei}
...
A manuális tesztelés folyamán többféle felhasználói szcenárió került vizsgálatra annak biztosítására, hogy a rendszer működése a valós használati körülmények között is megfelelő legyen. Egyes tesztesetekhez, ahol releváns, ábrák is tartoznak, amelyek szemléltetik a rendszer megfelelő válaszait (pl. hibaüzenetek, keresési eredmények).

\begin{table}[H]
	\centering
	\begin{tabular}{|p{3cm}|p{4cm}|p{4cm}|c|}
		\hline
		\textbf{Leírás} & \textbf{Lépések} & \textbf{Elvárt Eredmény} & \textbf{Eredmény} \\
		\hline \hline
		Szöveg feltöltése cím nélkül & Szöveg feltöltése úgy, hogy a cím mező üres marad & Hibaüzenet jelenik meg, a feltöltés nem történik meg (\ref{fig:no-title} ábra) & Sikeres \\
		\hline
		Japán szöveg validáció hibás inputtal & Nem japán karaktereket tartalmazó szöveg feltöltése japánként & Hibaüzenet jelenik meg, a rendszer nem fogadja el a bemenetet (\ref{fig:no-jp-char} ábra) & Sikeres \\
        \hline
        Szöveg feltöltés (.txt) & Fájl feltöltése a rendszerbe & Szöveg feldolgozásra kerül, elemek létrejönnek & Sikeres \\
        \hline
        Keresés a dashboardon & Kulcsszó beírása a keresőmezőbe & Csak releváns találatok jelennek meg (\ref{fig:search} ábra) & Sikeres \\
        \hline
        Tanulási folyamat frissítés & Válaszadás egy tanulási elemre & Státusz és ismétlési intervallum frissül & Sikeres \\
        \hline
        Szótár napi tanulási limit beállítása & A napi új elemek limitjének módosítása, majd tanulás indítása több elérhető elemmel & A rendszer csak a beállított limitnek megfelelő számú új elemet jelenít meg & Sikeres \\
        \hline
        Érvénytelen napi tanulási limit (0 érték) & A napi új elemek limitjének 0-ra állítása a beállításoknál & A rendszer validációs hibát jelez, és nem engedi a mentést (\ref{fig:limit-error} ábra) & Sikeres \\
        \hline
        
	\end{tabular}
	\caption{Manuális tesztesetek}
	\label{tab:management}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{images/impl/manual_testing/no_title.png}
  \caption{Hibaüzenet cím nélküli bevitelnél}
  \label{fig:no-title}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{images/impl/manual_testing/no_jp_char.png}
  \caption{Hibaüzenet japán karaktert nem tartalmazó szöveg bevitelénél}
  \label{fig:no-jp-char}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{images/impl/manual_testing/search.png}
  \caption{Keresés után csak a releváns eredmények láthatóak}
  \label{fig:search}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{images/impl/manual_testing/limit_error.png}
  \caption{Hibaüzenet megjelenítése érvénytelen napi limit beállítása esetén}
  \label{fig:limit-error}
\end{figure}